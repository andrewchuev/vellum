...
- **PDF, stage 1**: the reader covers viewing, navigation, zoom and the table of contents. Still to come: word selection and dictionary lookup (Pdfium's text layer already supports hit-testing), in-document search, storage scanning of `.pdf` files with first-page covers, a night mode and margin cropping. PDF books currently appear in the library after being opened (with a placeholder cover).
